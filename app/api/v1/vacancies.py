...
async def get_session() -> AsyncGenerator[AsyncSession, None]: #AsyncSession
...
async def get_vacancy_endpoint(
    vacancy_id: int = Path(le=(2 ** 31) - 1, ge=0), # Аналогично external_id
    session: AsyncSession = Depends(get_session)
) -> VacancyRead:
...
async def update_vacancy_endpoint(
    ...
    # return await update_vacancy(      | Internal Server Error: 
    #     session, vacancy, payload     | отсутствует проверка payload.external_id на существование
    # )                                 | 
    ### Начало фикса ###                                            #
    try:                                                            #
