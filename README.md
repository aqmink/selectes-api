...
### Запуск
   1) Выполнил сборку и запуск
      ...
      - Решение: добавил параметр `extra="allow"` в `SettingsConfigDict` > app/core/config.py строчка 10
   2) Перезапустил приложение
      - Проблема: Возникла `asyncpg.exceptions.InvalidCatalogNameError`
      - Решение: было неверно указано имя переменной `DATABASE_URL`, исправил и перезапустил приложение *опечатка?)* > app/core/config.py строчка 17
 ### Отладка работы приложения
   1) Приложение запустилось с ошибкой парсера
      ...
      - Решение: Отсутсвовала проврека поля `ExternalVacancyItem.city` на возможное принятие типа `NoneType`, добавил проверку, `city` теперь может принимать значение `None`  > app/service/parser.py строчка 43
   2) Заметил что вакансии парсяться слишком часто:
      - Проблема: Вакансии прибывают каждые 5 секунд, не минут
      - Решение: В `AsyncIOScheduler.add_job` заменил параметр `seconds` на `minutes` > app/service/scheduler.py строчка 13
   3) В логах возникает предупреждение от планировщика о пропуске задач парсинга
      ...
      - Решение: В `AsyncIOScheduler.add_job` установил параметр `misfire_grace_time=30`, теперь задачи могут быть запущенны с опозданием до 30 секунд > app/service/scheduler.py строчка 16
### Проверка эндпоитов:
   ...
   2) Проверил `POST /api/v1/vacancies/` на созданик вакансии с существующим `external_id` (работает корректно) и `PUT /api/v1/vacancies/{id}` на измение `external_id` на существующий
      ...
      - Решение: Добавил проверку `external_id` на существование, если его ещё нет, вакансия измениться, если уже есть отправиться ошибка аналогично `POST /api/v1/vacancies/` > app/api/v1/vacancies.py строчка 72
   3) Прверка на дурака:
      ...
      - Решение: Добавил валидатор для `vacancy_id` и `external_id`, ограничив их (поставив допустимый диапазон от 0 до 2 ^ 31 - 1) > app/api/v1/vacancies.py строчки 37, 63, 90 и app/schemas/vacancy.py строчка 15
   4) Проверка эндпоинта `POST /api/v1/parse/` и парсера
      ...
      - Решение: Убрал строчки, отвечающие за обновление вакансии из `upsert_external_vacancies` > app/crud/vacancy.py строчка 77
...
### Ещё некоторые наблюдения:
   ...
   5) Статус коды должны быть соответсвующими, возможно, стоило вынести это в баг, в `POST /api/v1/vacancies/`, если создается вакансия с существующим external_id, ответ не должен быть со статусом 200 *не 500, уже что-то))*
