...
## - Запуск
   ### 1) Выполнил сборку и запуск
      ...
      Решение: добавил параметр extral="allow" в SettingsConfigDict
   ### 2) Перезапустил приложение
      Проблема: Возникла asyncpg.exceptions.InvalidCatalogNameError
      Решение: было неверно указано имя переменной DATABASE_URL, исправил и перезапустил приложение
...
## - Проверка эндпоитов:
  ...
  ### 3) Прверка на дурака:
      ...
      Решение: Добавил валидатор для vacancy_id и external_id, ограничив их (я поставил диапазон от 0 до 2 ^ 31 - 1)
  ### 4) Проверка эндпоинта `POST /api/v1/parse/` и парсера
      Проблема: Изменение данных вакансии без изменения external_id, может привести к неожидаемому поведению, при парсинге проверяется, существуют ли в бд вакансии с таким же external_id, как у публичных вакансий, если они есть, то выполняется обновлене вакнсии в бд, её поля заменяются на поля публичной вакансии, что может привести к неожидаемому результату: изменённая через `PUT /api/v1/vacancies/{id}` после парсинга будет заменена на публичную вакансию (обновление вакансии производиться POST методом является нарушением правил построений RESTful приложений)
